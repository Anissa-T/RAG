...
def split_documents(documents, chunk_size=1000, chunk_overlap=200):
    ...
    text_splitter = RecursiveCharacterTextSplitter( #stratégie recursive le système regarde la pertinence du chunk d'une façon récursive 
...
def create_vectorstore(chunks, embedding_model_name="sentence-transformers/all-MiniLM-L6-v2"):
    ...
    try:
        ...
        # Create and save the FAISS index (vectoriser nos fragements textuels, mecanisme d'indexation ils vont être stockés gâce à la procédure de clusterisation)
...
#FAISS c'est le mecanisqme d'indexation 
# le 
...
def load_cpu_friendly_llm(model_id="Qwen/Qwen2.5-1.5B-Instruct"):   # "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
    """Load a small language model that can run on CPU""" # pour la génération de texte
    try:
...
if __name__ == "__main__":#permet de lancer le script il necessite deux arguments 
    import argparse
...
#model_id = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"  # Remplacez par le modèle que vous utilisez
#print(f"Testing text generation pipeline with model: {model_id}")
...
#try:
    ...
    #prompt = "Qui est Charles de Gaulle ?"
    #result = text_gen_pipeline(prompt, max_length=100, num_return_sequences=1)
    #print("\nGenerated Text:")
    #print(result[0]["generated_text"])
#except Exception as e:
    #print(f"Error testing text generation pipeline: {e}")


#bonne pratique : enlever les fichiers tenseurs => vider le cache
#torch.cuda.empty_cache()



# QWEN à la place de TinyLlama
# "Qwen/Qwen2.5-1.5B-Instruct"



# Rendu à minuit du 23 au 24 mai
# Le script et le rapport
# Rapport de 3 ou 4 pages
    # Expliquer les données utilisées, les méthodes et expériences menées, les résultats obtenus et les observations
        # Si on a eu des problèmes, exemple  hallucinations, le dire, essayer d'expliquer pourquoi, et dire comment on a fait pour régler le problème
        # Etre très précis et factuel : pas intéressant de parler de généralités et des banalités, parler de notre travail concrètement
        # Faire toutes les expériences possibles et imaginables qui sont intéressantes selon notre script, nos données et ce qu'on obtient comme observations et résultats : par exemple si on remarque qu'on pose une question dans une langue et qu'il répond dans une autre, on peut essayer de lui demander de répondre dans la même langue pour voir (rajouter cette demande au prompt) puis de lui préciser la langue, et pareil pour les données qu'il utilise pour le RAG (pourquoi toujours le français ?? Utilise les données dans ta langues, puis on lui donne directement la langue des données à utiliser)
